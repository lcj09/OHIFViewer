...
export default function getToolbarModule({ commandsManager, servicesManager }) {
  return [
    ...
    // 2026-04-28 - TMTV专用布局选择器：仅显示融合相关布局和三维布局
    {
      ...
      defaultComponent: props => TmtvLayoutSelector({ ...props, commandsManager, servicesManager }),
    },
    // 2026-05-22 - TMTV手动微调菜单组件
    {
      name: 'ohif.fusionAdjustMenu',
      defaultComponent: props => FusionAdjustMenu({ ...props, commandsManager, servicesManager }),
    },
    {
      name: 'ohif.saveMenu',
      defaultComponent: props => SaveMenu({ ...props, commandsManager, servicesManager }),
    },
    // [2026-07-01 新增] 覆盖层菜单 - 控制十字线和患者信息显示
    {
      name: 'ohif.overlayMenu',
      defaultComponent: props => OverlayMenu({ ...props, commandsManager, servicesManager }),
    },
    // [2026-08-06 新增] 同步设置菜单 - 控制方位切换是否同步
    {
      name: 'ohif.syncMenu',
      defaultComponent: props => SyncMenu({ ...props, servicesManager }),
    },
    // [2026-07-06 新增] 伪彩色菜单 - 切换PT volume的colormap
    {
      name: 'ohif.colormapMenu',
      defaultComponent: props => ColormapMenu({ ...props, commandsManager, servicesManager }),
    },
    // [2026-07-08 新增] SUV阈值菜单 - 快速设置PET窗位(体部SUV=6/头部SUV=10)
    {
      name: 'ohif.suvThresholdMenu',
      defaultComponent: props => SuvThresholdMenu({ ...props, commandsManager, servicesManager }),
    },
    // [2026-08-20 新增] 调窗下拉菜单 - 含手动调窗与CT预设
    {
      name: 'ohif.tmtvWindowLevelMenu',
      defaultComponent: props => WindowLevelMenu({ ...props, commandsManager, servicesManager }),
    },
    ...
    //   - 非 TMTV 布局：visible 控制 Cornerstone CrosshairsTool
    {
      ...
      evaluate: () => {
        try {
          ...
          const isActive =
            tmtvCrosshairService.getVisible() && !crosshairDisplayService.isSingleLineMode();
          return {
            disabled: false,
            isActive,
          };
      ...
      evaluate: () => {
        try {
          return {
            disabled: false,
            isActive:
              crosshairDisplayService.isVisible() && crosshairDisplayService.isSingleLineMode(),
          };
        } catch (e) {
          return { disabled: false };
        }
