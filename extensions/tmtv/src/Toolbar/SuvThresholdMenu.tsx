        ...
        if (vol?.metadata?.Modality === 'PT') {
          ...
          if (voiRange) {
            ...
            const matched = SUV_PRESETS.find(
              p => Math.abs(windowCenter - p.suvValue) < 0.1 && Math.abs(windowWidth - p.windowWidth) < 0.5
            );
            if (matched) {
              setCurrentPreset(matched);
            } else if (Math.abs(windowWidth - 2 * windowCenter) < 0.5 && windowCenter > 0) {
              // 匹配自定义模式（W ≈ 2 × L）
              setCurrentPreset({ id: 'custom', label: 'Custom', suvValue: Math.round(windowCenter * 10) / 10, windowWidth: Math.round(windowWidth * 10) / 10 });
            } else {
          ...
          <div className="flex flex-col gap-0.5">
            {SUV_PRESETS.map(preset => (
              <Button
                ...
                className={`flex h-9 w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-primary-dark ${
                  currentPreset?.id === preset.id ? 'text-common-bright bg-primary/20' : 'text-gray-400'
                }`}
            ...
            {!showCustomInput ? (
              <Button
                variant="ghost"
                className={`flex h-9 w-full items-center justify-between px-3 py-1.5 text-sm hover:bg-primary-dark ${
                  currentPreset?.id === 'custom' ? 'text-common-bright bg-primary/20' : 'text-gray-400'
                }`}
              ...
              <div className="flex items-center gap-1 px-2 py-1.5">
                <span className="text-xs text-gray-400 whitespace-nowrap">SUV</span>
                <input
                  ref={inputRef}
                  type="number"
                  min="0.1"
                  ...
                  className="h-7 w-16 rounded border border-white/20 bg-black/30 px-1.5 text-xs text-white outline-none focus:border-highlight"
                  placeholder="0.0"
                />
                <button
                  type="button"
                  className="h-7 min-w-[44px] shrink-0 rounded px-2 text-xs text-highlight hover:bg-primary-dark"
                  onClick={e => {
                    e.stopPropagation();
                    e.preventDefault();
                    handleCustomConfirm();
