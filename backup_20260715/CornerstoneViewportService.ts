...
class CornerstoneViewportService extends PubSubService implements IViewportService {
  ...
  public destroy() {
    ...
    // CRITICAL: Call cache.purgeCache() to clean up all cornerstone caches
    // including texture cache, metadata cache, and any other internal caches.
    // _purgeCacheRobust() only cleans _volumeCache and _imageCache, leaving
    // ~1,277 MB of residual data (texture cache, etc.) that causes memory leaks.
    try {
      cache.purgeCache();
      console.log('[ViewportService] cache.purgeCache() called');
    } catch (e) {
      console.warn('[ViewportService] cache.purgeCache() failed', e);
    }
    ...
    if (volumesNotLoaded.length) {
      ...
      volumesNotLoaded.forEach(volume => {
        if (!volume.loadStatus?.loading && volume.load instanceof Function) {
          // 性能优化：基于modality决定加载策略
          // CT volume：立即加载，保持渐进式渲染(2%)，让用户尽快看到CT图像
          // PT volume：延迟1.5s加载，禁用渐进式渲染，只在加载完成时触发一次渲染
          // 这样CT先显示，PT后台加载，总渲染回调从~100次降到~50次

          // 通过volumeId查找对应的volumeInput以获取modality
          const volumeInput = volumeInputArray.find(vi => vi.volumeId === volume.volumeId);
          const modality = volumeInput?.modality;
          const isPT = modality === 'PT';

          const startLoad = () => {
            if (volume.loadStatus?.loading || volume.loadStatus?.loaded) {
              return;
            }

            volume.load();

            // PT volume禁用渐进式渲染：只在加载完成时触发一次渲染
            // 注意：必须在load()之后设置，因为load()内部会重置reRenderFraction为2%
            if (isPT) {
              const totalFrames = volume.totalNumFrames || volume.imageIds?.length;
              if (totalFrames && totalFrames > 0) {
                volume.reRenderFraction = totalFrames;
                volume.reRenderTarget = totalFrames;
              }
            }
            // CT volume保持默认reRenderFraction(2%)，渐进式渲染让用户尽快看到图像
          };

          if (isPT) {
            // PT延迟1.5s加载，让CT先开始下载和解码
